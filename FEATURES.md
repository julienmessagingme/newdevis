...
- **Pages métier** (`/observatoire/metiers/{slug}` × 28) — **refondues le 2026-09-07** : au lieu de quatre moyennes toutes unités confondues (« 1 397 € de panier moyen, et alors ? »), la page ouvre sur **un chiffre à retenir** (« Fenêtre PVC posée : de 910 à 2 968 € — un rapport de 3,3 entre les artisans, sur 21 devis ») puis déroule les **prix poste par poste, à unité égale**, chacun étiqueté *fourniture + pose* / *pose seule* / *périmètre non précisé*, avec les fourchettes P10–P90 et le nombre de devis. Un bloc « Comment lire ces prix » explique ce qui fait varier un prix à prestation identique (caractéristiques du produit, dimensions cachées derrière un prix « à l'unité », dépose et finitions, taux de TVA) — et pourquoi les deux fourchettes ne se soustraient pas. Une page sans poste suffisamment observé n'affiche **aucun chiffre** et le dit. Contenu **rendu en HTML au build** (plus d'île React) : indexable, zéro JS.

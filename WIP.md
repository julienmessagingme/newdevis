...
**✅ Monétisation LIVE (2026-06-14, commits cfa3845..bb708fa) :** Stripe complet (`/api/gmc/create-checkout-session` mode subscription + coupon -50% 1er mois `duration:once` mensuel only, `/api/gmc/create-portal-session`, `/api/gmc/status` autoritaire serveur exposant `paymentsLive`, `stripe-webhook` routé par `metadata.product` → `gmc_subscriptions` branche VMD préservée, libs `gmc-stripe-config`/`gmc-status-compute`/`gmc-subscription`=`getGmcStatus`, migration `past_due`). **Page de vente `/gmc-abonnement`** (mensuel/annuel 12/120·25/210, déjà-abonné→portail, noindex). **Gate 2e chantier** (gratuit/essai/Essentiel=1, Multi payant=illimité ; 3 couches : garde backend `sauvegarder.ts` 403 `multi_required`, carte `AddChantierCard` verrouillée, garde montage `NouveauChantier` ; **conditionné à `GMC_PAYMENTS_LIVE`** = présence des price env vars, donc zéro régression avant go-live). **Bloc « Mon abonnement »** (paramètres cockpit) + **bandeau d'essai** (hub). **Emails — les 21 templates câblés** (2026-06-14) : `gmc-email-scheduler` pilote tout le cycle de vie (engagement J1-14, `reengage` chantier inactif >5j, conversion J-7/J-3/J-1/fin -50%, winback J+3/10/21, `paid_welcome`/`paid_onboard` J+2/`paid_checkin` J+14, `multi_nudge` Essentiel J+30, `renewal_notice` annuel J-3, `dunning`, `goodbye`) + `upsell_multi` sur tentative de 2e chantier (flag `multi_intent_at` posé par le gate). Ancres "payant" = `sent_at` du `paid_welcome` ; renewal dédupé par période. **Go-live fait** : produits+4 prix+coupon `Nb2ITi2O` en Live Stripe, env vars sur Vercel, `paymentsLive:true` confirmé prod. **E2E sandbox OK** (Stripe CLI), a attrapé+corrigé le bug `current_period_end` (API Stripe 2025+ : champ déplacé sur l'item → 500 webhook).

**✅ Suite (2026-06-14) :** **lecture seule J30** (écritures bloquées via `hasGmcWriteAccess` + gate dans `requireChantierAuth`/`requireChantierAuthOrAgent` + `sauvegarder`/`generer`/`ameliorer` ; method-aware, GET libres, bypass agent ; bandeau « essai terminé » rouge dans le cockpit) ; **message explicite « 1 chantier maximum »** (carte verrouillée + bannière `reason=multi`) ; **timeline de suivi** (table `gmc_subscription_events` écrite par le webhook + chronologie datée dans « Mon abonnement ») ; **comptes offerts** Julien + Johan en Multi (`signup_source=comp`) ; **fix toggle Canal WhatsApp IA** (lit l'état réel au montage via GET).

**✅ Clôture monétisation (2026-06-14, commits `4415634` + `87bed3a`) :** **cron `gmc-trial-expire-daily`** (07:50 UTC, jobid 32, flip trial→expired à J30 ; le scheduler route la suite winback par `trial_ends_at`, rejouable en 'trial' ou 'expired', et n'envoie `goodbye` que pour un abonné payant résilié = `stripe_subscription_id` présent) ; **test webhook auto** (Vitest, helpers purs extraits `subPeriodEndISO`/`gmcStatusFromStripe`/`planFromPriceId` dans `stripe-webhook-helpers.ts`, 12 tests, `npm test`) ; **upgrade Essentiel→Multi en place** (`/api/gmc/change-plan` via `stripe.subscriptions.update` + proration, câblé sur `/gmc-abonnement` carte Multi + box « déjà abonné », redirige vers `/mon-chantier/nouveau` si arrivé par le gate ; pas de config portail Stripe requise) ; **portail masqué pour comptes offerts** (`/api/gmc/status` + `getGmcStatus` exposent `isComp` + `hasStripeCustomer`) ; **reengage** sur `auth.users.last_sign_in_at` (vrai last-seen). **Prix Multi annuel 210 € confirmé** (Julien). Dry-run scheduler post-deploy = 200 OK (candidat `gmc_activate`). **+ UX cockpit P0** : `InsightsBanner` persistant (I3) + cartes mobile `IntervenantsListView` (N5b) ; N5c (touch Gantt) déjà fait (pointer events).

**🟠 Reste :** (1) `RESEND_API_KEY` sur **Vercel** (clé fournie 2026-06-14, à coller par Julien → emails payants temps réel via webhook au lieu du cron ≤24h + notif /avis) ; (2) **gros TODO** Paramètres agent (toggle IA auto-réponse artisans + OpenClaw). Voir `TODO.md` § GMC Monétisation.

**✅ Résolu 2026-07-03 :** vérification `invoice.subscription` v20+. Le helper `invoiceSubscriptionId` ([`stripe-webhook-helpers.ts`](src/lib/integrations/stripe-webhook-helpers.ts)) gère déjà les 4 emplacements possibles (ancien `invoice.subscription` string/objet, nouveau `invoice.parent.subscription_details.subscription`, replis `lines.data[0].parent.subscription_item_details.subscription` et `lines.data[0].subscription`) + est utilisé dans `stripe-webhook.ts:272` sur `invoice.payment_failed`. 18 tests unitaires passent. La branche past_due/dunning n'était donc PAS cassée — hypothèse WIP invalidée par la vérif. Le seul autre `.subscription` du webhook est `session.subscription` sur `checkout.session.completed` (champ stable, jamais retiré).

**⚠️ Ops :** `RESEND_API_KEY` à ajouter côté **Vercel** pour la notif `/avis` (présente sur Supabase, absente sur Vercel → notif muette, avis quand même stockés). Migrations GMC en SQL direct → `npx supabase migration repair --status applied 20260612120000 20260612130000 20260612140000 20260612150000 20260613090000 20260614100000 20260614170000 20260614190000 20260614210000 20260615100000` avant tout `db push`. ⚠️ `AddIntervenantModal` **plus dormante depuis 2026-07-01** — ravivée avec écran 2 contact (cf. section "Bug fix Ajouter un intervenant" plus haut). Ne pas suivre l'ancienne consigne "nettoyer".

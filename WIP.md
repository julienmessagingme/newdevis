...
- ✅ **Étape 1** — CPM généralisé. `forwardPass` extrait (coeur unique), `computePlanningDates` refactoré (comportement identique, verrouillé par test), `computeAdvancedPlanning` (graphe noeuds lot:`/`sub:`, dérivation conteneur min/max) + `computeAdvancedStartDateFromEnd`. Types `Subphase` + `PlanningEdge` (convention from=dependent/to=dependsOn documentée). **52 tests** `planningUtils.subphases.test.ts` (équivalence anti-régression + verrou comportement + sub→sub same/cross-lot + lot→lot mixte + cycle + délai). Build vert. **Revue faite** — 🟡 à cadrer étape 2 : l'API doit défaulter `duree_jours` sur sous-phase (sinon lot conteneur sans dates) + interdire lot↔sa propre sous-phase.
- ✅ **Étape 1bis** — seam d'habilitation premium. `src/lib/auth/advancedPlanningAccess.ts` (`getAdvancedPlanningAccess`/`canUseAdvancedPlanning`/`requireAdvancedPlanning`, habilités V1 = admin + allowlist GMC, 1 seul TODO pour brancher le tier abonnement). Endpoint `GET /api/gmc/advanced-planning-access` + hook `useAdvancedPlanningAccess`. **10 tests**. **Revue faite** : anti-bypass (lit la DB, pas le JWT), pas d'import circulaire (requireAdvancedPlanning mis dans le fichier domaine façon adminAuth.ts, PAS apiHelpers), gate serveur = vraie sécurité / hook = cosmétique.
- ✅ **Étape 2** — API CRUD sous-phases + recompute unifié. `planningServer.ts` (loaders + `recomputeChantierDates` subphase-aware + `wouldCreateCycle`). `planning.ts` : GET/PATCH enrichis (`subphases`/`subphaseDeps`) + recompute inline remplacé par le helper (behavior-preserving sans sous-phase). 3 routes : `POST lots/[lotId]/subphases`, `PATCH+DELETE subphases/[subId]`, `POST+DELETE subphases/deps`. Toutes premium-gated. Gardes : anti-cycle (même normalisation CPM), lot↔sa-propre-sous-phase, appartenance chantier, doublon→409, défaut `duree_jours=1`. **Revue faite**, colonnes SQL validées contre DB, build vert. ⚠️ Pas encore E2E (aucune UI ne les appelle → étape 4).
- ✅ **Étape 3** — hook `usePlanning` étendu. State `subphases`/`subphaseDeps` (peuplés depuis GET ET PATCH — sinon le setState complet effaçait les sous-phases). Actions `addSubphase`/`updateSubphase`/`deleteSubphase`/`addSubphaseDep`/`removeSubphaseDep` (non-optimistes : endpoint + refetch silencieux autoritatif, retournent `{ok,error}` pour afficher le refus cycle). `updateEndDate` subphase-aware (`computeAdvancedStartDateFromEnd`). **Revue faite** : flash loading corrigé (silent refetch), anti-rollback préservé, actions lot inchangées. Build vert.
- 🟢 **Étape 4** — UI livrée (build vert, revue faite), **QA navigateur à faire par Julien** (premium-gated à son compte). Toggle Simplifié/Avancé dans `PlanningChantier` (cadenas + upsell si non premium, préférence persistée localStorage, restaure l'avancé dès que `access.allowed`). Nouvelle vue `SubPlanningView` (timeline % lots + sous-bandes, lecture seule) + `SubphasePanel` (ajout/édition/suppression sous-phases + constructeur de dépendances cross-métier avec affichage des refus cycle/lot↔propre-sous-phase). **Vue simplifiée `PlanningTimeline` strictement inchangée** (zéro régression). Limites V1 : pas de D&D des sous-phases (gestion par panneau), pas d'axe de dates dans l'avancé.
- ⬜ Étape 5 — polish + doc (+ QA navigateur étape 4) · Phase 2 (différée) — agent IA

...
**🟠 Reste :** **lecture seule J30** (bloquer les mutations, pas juste le bandeau — avant l'expiration des 1ers essais ~juillet ; le « plan figé Phase 2 » du TODO reste la réf) ; confirmer **prix Multi annuel = 210 €** (pas 250) dans Stripe Live ; `RESEND_API_KEY` sur **Vercel** → emails payants temps réel via webhook au lieu du cron ≤24h (+ notif /avis) ; **test webhook auto** (`subPeriodEndISO` + routage) ; cron `trial→expired` à J30 ; affiner `reengage` (vrai last-seen plutôt que `chantiers.updated_at`) + chemin d'upgrade **Essentiel→Multi** (le checkout 409 sur un abonné actif → passer par le portail Stripe, vérifier qu'il autorise le changement de plan). Voir `TODO.md` § GMC Monétisation.

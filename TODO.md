...
- [ ] 🟡 **LE LOGO DES AUTRES MAILS VMD EST EN BASE64, QUE GMAIL NE REND PAS** (constaté le 21/09 en réparant le mail J+15). `supabase/functions/_shared/vmd-emails.ts` embarque le logo en `data:image/png;base64` dans ses gabarits (welcome, négociation, comparaison…) : c'est robuste au blocage d'images distantes, mais **Gmail ignore les data URI dans les e-mails** — le logo y est donc probablement invisible depuis toujours. Le mail J+15 utilise désormais l'URL `/email/logo-vmd-icon.png`, qui fonctionne partout où les images distantes sont autorisées. ⚠️ **Aucun des deux motifs n'est bon partout** : à trancher après un test réel dans Gmail, Outlook et Apple Mail — pas au jugé. Et le PNG fait 32×32 alors qu'il est affiché en 40 : un 64×64 donnerait un rendu net.

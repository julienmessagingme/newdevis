...
- [ ] 🔴 **TRANCHER LA CONCURRENCE ENTRE TROIS SITEMAPS** — `sitemap-index.xml` (98 URLs, auto-généré, à jour), `sitemap-blog.xml` (74 articles, SSR, déclaré le 21/09) et **`sitemap.xml` (107 URLs, liste écrite À LA MAIN)**. Ce dernier n'est PAS déclaré dans `robots.txt` et ne doit pas l'être en l'état : ses 16 pages statiques sont périmées (il ignore les 55 pages de l'observatoire, les guides, le centre-aide). ⚠️ **Vérifier dans Search Console s'il y a été soumis manuellement** — si oui, Google reçoit deux listes contradictoires. Les options : le supprimer, ou le réécrire pour qu'il lise les routes au lieu d'une liste en dur.

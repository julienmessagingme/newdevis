...
- [ ] 🟡 **LE SITEMAP ET LE CANONICAL NE DÉSIGNENT PAS LA MÊME URL — trouvé le 21/09 en supprimant le sitemap manuel** : `sitemap-0.xml` déclare **98 URLs sur 98 avec un slash final** (`/faq/`), alors que le canonical de chaque page est **SANS slash** (`/faq`, construit par `BaseLayout` depuis `Astro.url`). Les deux formes répondent **200 avec un contenu identique à l'octet près**. On demande donc à Google d'explorer 98 adresses dont chaque page lui dit ensuite « la vraie est ailleurs » — très probablement la source des lignes « Page en double » et « Autre page avec balise canonique correcte » du rapport Search Console. `trailingSlash` n'est configuré nulle part dans `astro.config.mjs`. **Deux façons de le fermer** : aligner le sitemap sur le canonical (option du plugin), ou aligner le canonical sur le sitemap. ⚠️ **Mesurer avant** : changer la forme déclarée de 98 URLs n'est pas un ajustement cosmétique, et le sitemap manuel qu'on vient de retirer était le seul à déclarer la forme canonique.

...
- [ ] 🟡 **TROIS ACCENTS ORANGE DES MAILS VMD RESTENT SUR L'ANCIENNE TEINTE** — `#F58A06` sur une bordure d'encadré, le label « Offre réservée aux utilisateurs VMD » et un prix, dans `_shared/vmd-emails.ts`. Ce ne sont ni le logo ni l'écriture, donc hors de l'harmonisation du 21/09 ; mais l'orange du site est **`#F97316`** et **l'écart se voit dans le même mail** (le `.fr` de l'en-tête est déjà à la bonne teinte). ⚠️ L'encadré ambre `#FEF3C7` est accordé à l'ancienne teinte : changer l'un sans regarder l'autre déséquilibrerait le bloc. **Décision de charte, pas correctif.**

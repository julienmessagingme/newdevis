...
- 🔴 **LE TITRE ET LE MESSAGE DE L'EXPERT PEUVENT SE CONTREDIRE (trouvé le 23/09 en rendant les 200 cartes)** — sur le devis ALES (`devis sdb.pdf`, `corrected`), le hero annonce **« Environ 1 332 € à discuter avec l'artisan »** pendant que l'encadré juste en dessous dit *« nous vous recommandons de **ne pas signer** ce document en l'état »*. L'expert a réécrit le TEXTE sans que le verdict stocké suive : `decisionAffichee` lit `verdict_decisionnel` / `criteres_rouges`, qui disent toujours « négocier ».
  ⚠️ **Ce n'est PAS un défaut d'affichage** — le composant fait ce qu'on lui donne. C'est le chemin de correction experte (`decide.ts`) qui laisse diverger le verdict et le message. **À instruire à part** : soit l'écran de revue impose de trancher le verdict quand un message est écrit, soit `resyncVerdictLigne` le dérive du message (il en extrait déjà la première phrase depuis le 05/09). Population à mesurer avant de choisir.

...
- [ ] 🔴 **65 pages d'analyse promettent « une réponse sous 24 h ouvrées » depuis quatre mois** — et masquent leur montant (`provisoire` sur `pending_review`, règle du 30/08). Ces devis ont été déposés entre avril et juillet ; c'est notre régénération du 15/09 qui les a remis en `pending_review`. Ils sont désormais **différés** (hors de l'écran de revue, migration `20260917220000`) mais **leur page publique n'a pas changé** : le bandeau bleu et le masquage du montant sont toujours là.
  - ⚠️ **C'est une décision produit, pas un correctif** : basculer leur `review_status` ferait réapparaître un montant chiffré sur 65 analyses **sans qu'aucun expert ne les ait relues**, alors qu'elles ont été signalées pour des signaux à risque. Les trois options, à trancher : (a) les laisser en l'état, (b) reformuler le bandeau quand l'analyse a plus de 30 jours (« relecture non prioritaire », sans promesse de délai), (c) les repasser en `auto_approved` et assumer que le moteur d'aujourd'hui vaut mieux que celui qui les a flaggées.
...
- [ ] 🟡 **Remettre les différées dans la file** quand on voudra les instruire : `update public.analyses set review_differe_le = null where review_status = 'pending_review' and review_differe_le is not null;` — 30 d'entre elles portent des postes accusés (67 postes, 116 974 €), dont un seul devis (`DV0003541`, 02/06) en porte 17.

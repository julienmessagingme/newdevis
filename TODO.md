...
- ✅ **FAIT le 25/09 — `/pass-serenite` ne porte plus qu'un `<h1>`.** Le hero de l'île React passe en `<h2>`, le titre statique garde le H1. **Le sens de l'arbitrage tient en une ligne** : le composant est monté en `client:only`, donc son titre est absent du HTML servi — mettre le H1 là laisserait la page sans aucun titre de niveau 1 à l'indexation. Rendu inchangé (48px/700 vérifié en calculé).
- 🔴 **`analyses.work_type` EST NULL SUR 470 ANALYSES SUR 472**, et la dernière renseignée date du **12 février 2026**. C'est ce qui a tué le widget « social proof » pendant sept mois (cf. `CLAUDE.md`), et c'est aussi ce que documentait déjà le 08/09 : *« `work_type` et `domain` sont vides ou constants sur le stock »*. ⚠️ **Le champ est donc inexploitable pour toute segmentation** — observatoire, statistiques par métier, ciblage. À instruire : d'où devait-il venir (choix utilisateur à l'upload ? `detectChantierType` ?), et depuis quand la chaîne est rompue.

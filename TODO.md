...
- [ ] 🟡 **LE REPLI PAR NOM N'UTILISE PAS LE CODE POSTAL (2026-09-13)** — la requête est `q=<nom>` seul ; `pickBestNameMatch` ne départage qu'**après**, sur les 5 résultats renvoyés. Sur « Entreprise FK » à Saint-Germain-du-Puy (18390), l'API a rendu un taxi parisien, une SCI en Isère et une activité de courrier à Asnières — et rien d'autre à proposer. Vérifié à la main : `q=Entreprise FK&code_postal=18390` renvoie **0 résultat**, ce qui est la bonne réponse. L'API `recherche-entreprises` accepte `code_postal` et `departement` ; le code postal du devis est déjà extrait (`client.code_postal`, et l'adresse de l'entreprise). ⚠️ Attention au cas où l'artisan travaille hors de son département — filtrer **en second passage** (d'abord avec le code postal, repli sans) plutôt que d'exclure d'emblée. À mesurer sur les 8 recherches ambiguës du stock.

...
- [ ] **Bump `ENGINE_VERSION` pour appliquer la garde de couverture au STOCK (2026-09-04)** : la garde « on n'affirme pas une conformité sans avoir comparé » ne s'applique qu'aux conclusions REGÉNÉRÉES. Les **14 analyses** mesurées qui disent « dans la norme » avec 0 % de couverture gardent leur verdict tant que `ENGINE_VERSION` (`src/pages/api/analyse/[id]/conclusion.ts`) reste à `1.1.0-refonte`. Un bump les régénère à leur prochaine visite. ⚠️ Deux effets à assumer : régénération massive du cache, et **retour en `pending_review` des analyses à signaux risqués** (Piste C by design) — donc file de revue à absorber. Décision Johan : la règle CLAUDE.md interdit de bumper pour un cas isolé, celle-ci est structurelle, mais le moment se choisit.

...
- [ ] 🔴 **LA FILE DE REVUE SE BOUCHE : 76 EN ATTENTE CONTRE 43 STATUÉES (2026-09-16)** — 33 corrigées + 10 validées + 0 rejetée, dernière décision le **14/09**. Chaque analyse en `pending_review` est de la matière d'apprentissage **non récoltée**, et la file grandit plus vite qu'elle ne se vide. Deux conséquences : le gold standard stagne, et le bandeau bleu « validation expert en cours sous 24h » devient une promesse fausse pour 76 utilisateurs.
  - ⚠️ À trancher : resserrer les déclencheurs de revue (le `ratio_aberrant` ignore encore les groupes en confiance non-haute — déjà au backlog), ou assumer un rythme de revue et le dire autrement à l'utilisateur.

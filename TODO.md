...
- [ ] 🔴 **PERSISTER L'ÉTALON HUMAIN EN BASE (2026-09-10)** — les 150 jugements de Johan vivent dans deux fichiers sur son poste (`relecture-reponses.json`, `relecture-rapprochement.cle.json`), ignorés par git parce qu'ils contiennent des lignes de devis de clients et que le dépôt est **public**. C'est le seul actif qui permette de mesurer la qualité du rapprochement : le perdre, c'est refaire un après-midi de relecture. **Table Supabase** (`match_gold_standard` : ligne de devis, texte de requête, candidats, réponse humaine, date, relecteur), plus un script d'import et un script de scoring. À faire avant toute modification du matcher.

- [ ] 🔴 **COMPLÉTER LE CATALOGUE SUR LES TROIS FAMILLES NOMMÉES PAR LA RELECTURE (2026-09-10)** — 36 % des lignes jugées n'ont **aucune** bonne réponse parmi les cinq candidats, et le relecteur a nommé les manques : (a) **les déposes** — 5 occurrences sur 150, le catalogue n'a presque que des poses ; (b) les **lignes de chantier non-travaux** (démarches administratives, Consuel, Dommage-Ouvrage, éco-participation, suivi de travaux, livraison, location de matériel) — une quinzaine d'occurrences : à décider si on les couvre ou si on les EXCLUT explicitement du chiffrage, ce qui serait plus honnête ; (c) des **variantes matière** absentes — gouttière aluminium (×2), menuiserie avec volet roulant intégré (×2). ⚠️ Chaque ajout se vérifie ensuite sur l'étalon, pas au jugé.

- [ ] 🟢 **RE-CLASSER LE TOP-5 AU LIEU DE PRENDRE LE TOP-1 (2026-09-10)** — la piste que le banc d'embedding a fait apparaître, et la seule qui reste avec un gisement chiffré. Sur l'étalon de 317 paires, la bonne entrée est **1re dans 56 % des cas mais dans les 5 premières dans 86 %** : la recherche vectorielle TROUVE, elle ne CLASSE pas. `matchSingleLineVectorial` prend aujourd'hui le top-1 et descend la liste tant qu'une garde rejette — c'est un filtre binaire, pas un classement. Piste : **scorer les 5 candidats** en combinant le cosinus avec les signaux qu'on calcule déjà (recouvrement lexical, compatibilité d'unité, métier exclusif, plausibilité du ratio de prix) et garder le meilleur score. ⚠️ **La validation ne peut PAS se faire sur l'étalon lexical du banc** (il est construit lexicalement, ce serait circulaire) : il faut un échantillon de ~150 paires relu à la main, tiré au sort dans la bande de confiance moyenne. Prévoir cette relecture AVANT d'écrire le code, sinon la mesure ne prouvera rien.

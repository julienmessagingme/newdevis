...
- [ ] 🟡 **RÉCUPÉRER UN SIREN DANS UN NUMÉRO DE LONGUEUR INATTENDUE (2026-09-13)** — `verify.ts` ne parse que 9, 13 (rattrapage) ou 14 chiffres. Un numéro de 10, 11 ou 12 chiffres ne déclenche **aucune recherche**. Mesuré sur les 13 numéros non retrouvés du stock : prendre les **9 premiers chiffres comme SIREN** quand ils passent la clé de Luhn ne récupère qu'**un seul cas** (HDH batiment, 12 chiffres → SIREN 851 828 566, entreprise réelle et active) — les autres échouent à Luhn ou ne renvoient rien. Gain réel mais unitaire : à faire si on repasse dans ce code, pas pour lui-même. ⚠️ Ne JAMAIS étendre au-delà de la clé de Luhn : tronquer un numéro au hasard jusqu'à tomber sur une entreprise est le meilleur moyen d'en désigner une au hasard.

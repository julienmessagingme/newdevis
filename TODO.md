...
- [ ] 🔴 **RÉÉCRIRE LES LIBELLÉS DU CATALOGUE — c'est le levier mesuré (2026-09-10)** : l'entonnoir de la bande de confiance moyenne (854 groupes, 814 942 €) dit que **54 % des groupes échouent parce que le nom de tête de notre libellé est absent de la ligne de devis**. Trois causes, toutes dans NOS libellés : une tête de catégorie (« **Porte** coulissante galandage » quand le devis écrit « Coulissant à galandage 2 vantaux »), un préfixe générique (« **Travaux** de préparation et application de deux couches de peinture » contre « Mise en Peinture Calista Mat »), et la morphologie (« coulissante » ≠ « coulissant » — la lemmatisation ne coupe que le -s/-x final). **Deux pistes à mesurer, pas à supposer** : (a) réécrire les libellés en commençant par le nom de l'ouvrage, jamais par la catégorie ; (b) ajouter une colonne d'alias (les formulations réelles relevées dans les devis) concaténée au texte embarqué. Chaque piste se mesure de la même façon que le correctif du 10/09 : nombre de groupes qui passent en confiance haute, **et relecture une par une des nouveaux rapprochements**. ⚠️ Ne PAS remettre d'énergie dans la règle de promotion : relâcher le nom de tête n'ajoute que 7 groupes dont 2 faux, c'est mesuré.

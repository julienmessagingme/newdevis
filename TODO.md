...
- [ ] **Mesurer le vrai plafond d'extraction sur un devis UNIQUE long (2026-09-07)** : le plafond de 8 pages vient d'une mesure faite sur un PDF **fusionné** (11 pages, plusieurs artisans, `AI_TIMEOUT` à 93 s). Un devis unique et structuré de 15 pages n'est pas le même exercice pour l'extraction — moins d'entités, un seul jeu de totaux. Le cas « Devis complets.pdf » le rend concret : après découpage, son second devis (D0174, 15 pages, un seul artisan) reste refusé alors qu'il est peut-être parfaitement analysable. **Mesure à faire** : lancer l'extraction sur ce fichier découpé (`seg-2`) et chronométrer. Si ça passe, relever `PAGES_MAX_EXTRACTION` — le découpage prend alors tout son sens puisqu'il produit justement des devis uniques.

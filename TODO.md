...
- [ ] 🟡 **Les 3 pages produit restent muettes, et c'est une décision à prendre** — `/premium`, `/suivi-budget` et `/calculette-travaux` servent elles aussi une coquille vide, mais `Premium` et `SuiviBudget` sont **réellement interactives** (9 et 8 occurrences d'état) : elles ne peuvent pas être basculées. Le choix est donc **de les sortir du sitemap** (on ne demande pas l'indexation d'une page qu'on sait vide) ou d'en extraire une coquille statique avec l'île à l'intérieur. Aucune des deux n'est un correctif de cohérence : ça se tranche.
...
- [ ] 🔴 **`/suivi-budget` ANNONCE `localhost` À GOOGLE — la page est inindexable** (trouvé le 21/09 en mesurant le slash). Son canonical vaut `https://localhost/suivi-budget` : la page est en `prerender = false` et **ne passe pas de canonical explicite** à `BaseLayout`, donc le repli sur `Astro.url` récupère l'origine interne du runtime Vercel — le piège déjà documenté dans `CLAUDE.md`. `/premium` et `/calculette-travaux` sont dans le même cas de rendu mais fournissent leur canonical, et sont donc correctes. **Une seule page concernée**, vérifié sur le build. Correctif évident (passer `canonical` comme les deux autres) mais **à vérifier en production après déploiement** : c'est le seul moyen de constater que `Astro.url` n'est plus lu.
- [ ] 🟡 **UN SEUL SITEMAP POUR DEUX DOMAINES — 18 pages GMC déclarées chez VMD, et GMC sans sitemap valide** (trouvé le 21/09). Deux faces du même défaut : (a) `sitemap-0.xml` de VMD déclare 18 pages (`/planning-chantier`, `/logiciel-suivi-chantier`, `/centre-aide`…) dont le canonical pointe **correctement** vers `gerermonchantier.fr` — on soumet dans la propriété VMD des pages qui disent appartenir ailleurs ; (b) le sitemap servi sur `gerermonchantier.fr` contient **98 URLs, toutes sur verifiermondevis.fr** (même fichier, généré avec `site: 'https://www.verifiermondevis.fr'`), donc **aucune page GMC n'est déclarée sur son propre domaine**. ⚠️ `@astrojs/sitemap` ne connaît qu'un seul `site` : séparer les deux demande soit un second sitemap (route SSR, comme `sitemap-blog.xml`), soit un filtre + réécriture dans `serialize`. **Décision produit avant code** — c'est le SEO de l'autre domaine.
...
- [ ] 🟡 **`/guides/` annonce « 891 prix marché »** alors que le catalogue en compte 926 — promesse chiffrée périmée, exactement ce que `CATALOGUE_TAILLE` (source unique depuis le 08/09) existe pour éviter. Vu en basculant la page le 21/09, laissé en l'état : c'est du contenu éditorial, hors périmètre d'un correctif de rendu.

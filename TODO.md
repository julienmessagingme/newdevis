...
- [ ] 🟢 **VOCABULAIRE FERMÉ POUR LA CATÉGORIE D'EXTRACTION (2026-09-11)** — le chiffre qui reste de la mesure ci-dessus : sur 100 lignes en **confiance haute** (des prix affichés) tirées au sort, la catégorie est **fausse dans 10 % des cas** et n'est qu'un titre de section dans 14 %. Cause : le gabarit JSON d'`extract.ts` dit littéralement `"categorie": "categorie"` — aucune consigne, aucun dictionnaire — et la règle (e) des devis à blocs menuiserie demande même de « classifier selon le TITRE du bloc ». Piste : imposer une **liste fermée de familles de métier** (celles de `market_prices.metier`, 33 valeurs) avec repli explicite. ⚠️ **Ça ne se mesure pas sur le stock** : les catégories y sont déjà écrites. Il faut re-extraire un échantillon de PDF réels avec et sans la liste et comparer — banc à construire, et à chiffrer (une extraction Gemini par devis et par variante). ⚠️ Et la mesure ci-dessus interdit d'en déduire un gain automatique sur le rapprochement : une catégorie plus propre n'est utile que si elle est plus JUSTE, pas seulement mieux rangée.

...
- [ ] 🟡 **VÉRIFIER SI `gerermonchantier.fr` A UNE PROPRIÉTÉ SEARCH CONSOLE** — le sitemap GMC existe désormais et est déclaré dans `robots.txt`, donc Google le lira en explorant le domaine. Mais **sans propriété GSC, on ne verra jamais ce qu'il en fait** : ni pages indexées, ni requêtes, ni erreurs. Le travail SEO sur GMC est aujourd'hui aveugle. Si la propriété n'existe pas, la créer et y soumettre `https://www.gerermonchantier.fr/sitemap-gmc.xml`.

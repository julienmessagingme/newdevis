...
- **embedding-001 déprécié (2026-05-21)** : `models/embedding-001` retourne désormais `404 "not found for API version v1beta, or is not supported for embedContent"`. Cause : Google l'a déprécié sans rétro-compat sur l'endpoint v1beta. **Modèles actuels** : `text-embedding-004` (768 dim, stable, recommandé court terme) ou `gemini-embedding-001` (3072 dim, GA août 2025, plus puissant mais demande une migration `vector(3072)` sur la colonne pgvector). Le script `scripts/seed_market_prices_embeddings.mjs` utilise désormais `text-embedding-004`. Si on switche un jour vers `gemini-embedding-001` : refaire la migration `ALTER TABLE market_prices ALTER COLUMN embedding TYPE vector(3072)`.

...
- 🔴 **UN PUSH FAIT AVEC `GITHUB_TOKEN` NE DÉCLENCHE AUCUN WORKFLOW `on: push`.** C’est un garde-fou GitHub contre les boucles, et il est fatal ici : le déploiement passe par `vercel-ci-deploy.yml`, déclenché sur push (l’intégration Git de Vercel est inopérante depuis que le dépôt est privé sur le plan Hobby). Un simple commit laisserait donc le JSON à quai — **exactement l’incident des « commits dormants » de mai 2026**. Le workflow appelle donc explicitement `gh workflow run vercel-ci-deploy.yml` après le push, ce qui exige la permission `actions: write`.

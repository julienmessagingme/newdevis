...
- **Catalogue market_prices trop grand pour Gemini** : 470+ entrées envoyées entières → Gemini invente des identifiants → tout finit dans "Autre". Solution : `filterRelevantPrices()` dans `market-prices.ts` pré-filtre via détection de domaines (180+ triggers mots-clés) → catalogue réduit à ~20-80 entrées avant l'appel Gemini. **Ne jamais supprimer ce filtre** : sans lui, le matching échoue systématiquement sur les devis multi-corps.

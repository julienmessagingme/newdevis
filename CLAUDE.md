...
- 🟡 **Phase 4 — tranche 1 LIVRÉE le 2026-08-15** : nouveau module déterministe [`src/lib/analyse/leviersBuilder.ts`](src/lib/analyse/leviersBuilder.ts) (17 tests vitest) → 2 nouveaux champs optionnels `ConclusionData.verdict_ligne` (verdict 1 ligne, **motif TOUJOURS nommé**, marge chiffrée) + `ConclusionData.leviers` (max 3, hiérarchisés puissant/important/bonus : entreprise > clauses rouges > quantités > espèces > acompte>50% > surcoût matériel > acompte 31-50% > clauses oranges > devis>12 mois > assurance > références fallback). Branchés dans `conclusion.ts` (chemin standard, best-effort try/catch) + hero `AvisSurLeDevis` (verdict_ligne prime sur les heuristiques) + nouveau composant `LeviersNegociation` dans `AvisEtPreparation`. Le levier « date devis > 12 mois » clôt la moitié Phase 4 du bug DEVIS-DATE. Fallback intégral pour les conclusions pré-Phase 4 (champs absents → UI historique). **Tranche 2 restante** : détail poste-par-poste replié par défaut, requalification du tableau de répartition (chips), message copiable aligné sur les leviers, leviers visibles sous bypass (leçon ZenCouverture), réconciliation fiche rendez-vous ↔ leviers, replays d'acceptance (Toiture Boxes / Maçonnerie / Mélier / DUBOIS), puis bump `ENGINE_VERSION` pour servir Phase 4 au stock.

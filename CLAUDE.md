...
- **Funnel / nav GMC (2026-06-18)** : « Tester gratuitement » (header `gmc-landing`) → **`/beta`**. `/mon-chantier` (hub `MonChantierHub`) **redirige vers le cockpit si exactement 1 chantier** (escape pour en créer un 2e = CTA « Créer avec l'IA » du cockpit → `/mon-chantier/nouveau`). « Mes chantiers » du header `gmc-landing` est visible pour **tout user connecté** (l'allowlist `hasGmcAccess` ne gate PLUS la nav UI ; l'accès réel reste serveur via l'abonnement). Section « Suggestions pour préciser votre projet » retirée de `ScreenPrompt` (UX confuse).

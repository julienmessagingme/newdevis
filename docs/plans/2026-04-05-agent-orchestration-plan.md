...
**Files:**
- Create: `src/pages/api/chantier/[id]/agent-insights.ts` *(note: `/insights` est déjà pris par les insights MOE Gemini du cockpit)*
- Create: `src/pages/api/chantier/agent-config.ts`

### Step 1: Agent insights route

Create `src/pages/api/chantier/[id]/agent-insights.ts` — GET (list + unread_count), POST (create from agent with dedup 24h), PATCH (mark read single/all).

POST accepts both `X-Agent-Key` (for edge functions) and JWT (for user). Dedup by title within 24h + unique index `idx_insights_dedup` (chantier_id, title, day).

**IMPORTANT pour les Tasks 3 et 4:** L'URL est `/api/chantier/${chantierId}/agent-insights` (pas `/insights`). L'agent-checks (Task 3) insère en DB directement via service_role — ce qui est OK mais doit gérer le conflit unique index (ON CONFLICT DO NOTHING). L'orchestrator (Task 4) doit aussi utiliser la bonne URL si appel API.
...
export interface ChantierContext {
  ...
  messages_since_last_run: Array<{
    ...
    group_name: string | null;   // WhatsApp group name (e.g. "Plomberie - Chantier Martin")
    is_owner: boolean;           // true if from_me (the chantier owner sent this)
    is_known_contact: boolean;
    contact_role: string | null;
  }>;
  // Owner's unanswered questions (from_me messages with no reply in the same group within 48h)
  owner_pending_questions: Array<{
    body: string; timestamp: string;
    group_name: string | null; inferred_lot: string | null;
  }>;
...
export async function buildContext(
  ...
  const [budgetRes, planningRes, contactsRes, paymentEventsRes, chantierRes, waMessagesRes, waGroupsRes, insightsRes] =
    await Promise.all([
      ...
      // Direct Supabase: WhatsApp messages since last run (include from_me + group_id)
  ...
  // ── Try to match group name → lot name (fuzzy: group "Plomberie - Chantier X" → lot "Plomberie") ──
  const groupJidToLot = new Map<string, string>();
  // Will be populated after enrichedLots is built (see below)
  ...
  for (const [jid, gName] of groupJidToName) {
    ...
    for (const lot of enrichedLots) {
      // Strip leading emoji/spaces from lot name (Unicode-aware — keeps accented chars: É, è, ç)
  ...
  const mappedMessages = waMessages.map((m: any) => {
    ...
    if (m.from_me) {
      return {
        ...
        matched_lot: groupLot,       // inferred from group name
    ...
    return {
      ...
      body: m.body ?? "",
      timestamp: m.timestamp,
      matched_lot: lotFromContact ?? groupLot,  // fallback: infer from group name
      group_name: groupName,
      is_owner: false,
  ...
  const ownerQuestions = mappedMessages.filter(m => m.is_owner && m.body.includes("?"));
  ...
  for (const q of ownerQuestions) {
    // Check if anyone replied AFTER this question in the same group
    const repliesAfter = mappedMessages.filter(m =>
      !m.is_owner &&
      m.group_name === q.group_name &&
      m.timestamp > q.timestamp
    );
    if (repliesAfter.length === 0) {
      // Also check older messages (beyond current run window) — look back 48h
  ...
  const lotPhones = enrichedLots
    .filter(l => l.contact_phone)
    .map(l => l.contact_phone!.replace(/^\+/, "").replace(/^0/, "33"));
  ...
  if (lotPhones.length > 0) {
    ...
    for (const msg of lastMsgs ?? []) {
      // Keep only the most recent message per phone (results ordered DESC)
  ...
  for (const lot of enrichedLots) {
    ...
    // Silent contact (5+ days without message) — uses batched Map, no N+1
...
A) Message du PROPRIÉTAIRE (marqué "👤 Vous (propriétaire)").
...
Chaque message arrive d'un groupe WhatsApp. Le nom du groupe est entre parenthèses (ex: "📱 Plomberie - Chantier Martin").
...
${ctx.messages_since_last_run.length > 0
  ? ctx.messages_since_last_run.map(m => {
      const groupTag = m.group_name ? ` 📱 ${m.group_name}` : '';
      if (m.is_owner) {
        return `[${m.timestamp}]${groupTag} 👤 Vous (propriétaire)${m.matched_lot ? ` → lot "${m.matched_lot}"` : ''} : "${m.body}"`;
      }

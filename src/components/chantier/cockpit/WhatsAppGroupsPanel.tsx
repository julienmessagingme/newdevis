...
import { MessageCircle, Copy, Check, ChevronDown, ChevronUp, Loader2, Users, Trash2 } from "lucide-react";
...
export default function WhatsAppGroupsPanel({
  ...
  async function handleDelete(groupId: string) {
    if (!window.confirm("Retirer ce groupe de l'app ? Le groupe WhatsApp ne sera pas supprimé.")) return;
    setDeletingGroupId(groupId);
    try {
      await fetch(`/api/chantier/${chantierId}/whatsapp-groups?groupId=${groupId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      onGroupCreated(); // re-fetch
    } finally {

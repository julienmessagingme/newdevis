  ...
  return <section className="py-20 bg-muted/50">
      <div className="container">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Chaque critère est évalué et justifié.</p>
        </div>

        {/* Scoring Levels */}
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {scoringLevels.map((level, index) => <div key={index} className={`${level.bgColor} ${level.borderColor} border-2 rounded-2xl p-6 text-center`}>
        ...
        <div className="bg-card rounded-2xl p-8 card-shadow">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {analysisBlocks.map((block, index) => <div key={index} className="text-center">
                ...
                <ul className="space-y-2">
                  {block.items.map((item, i) => <li key={i} className="text-sm text-muted-foreground flex items-center justify-center gap-2">

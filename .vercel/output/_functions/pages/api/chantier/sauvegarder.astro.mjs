...
const POST = async ({ request }) => {
  ...
  if (artisans?.length) {
    ...
    const artisansWithJobType = artisans.filter(
      (a) => a.job_type && a.quantite && a.quantite > 0
    );
    if (artisansWithJobType.length > 0) {
      const jobTypes = artisansWithJobType.map((a) => a.job_type);
      const { data: prices, error: pricesError } = await supabase.from("market_prices").select(
        "job_type, unit, price_min_unit_ht, price_avg_unit_ht, price_max_unit_ht,fixed_min_ht, fixed_avg_ht, fixed_max_ht, ratio_materiaux, ratio_main_oeuvre"
      ...
      if (pricesError) {
        ...
        for (const artisan of artisansWithJobType) {
          const price = priceMap.get(artisan.job_type);
          ...
          const rM = Number(price.ratio_materiaux ?? 0.4);
          const rMO = Number(price.ratio_main_oeuvre ?? 0.55);
          const ratioTotal = rM + rMO;
          const safeRM = ratioTotal >= 1 ? 0.4 : rM;
          const safeRMO = ratioTotal >= 1 ? 0.55 : rMO;
          const calcBudget = (unitP, fixed) => q * (Number(unitP) || 0) + (Number(fixed) || 0);
          ...
          const { error: updateError } = await supabase.from("lots_chantier").update({
          ...
          }).eq("chantier_id", chantierId).eq("nom", artisan.metier);
          if (updateError) {

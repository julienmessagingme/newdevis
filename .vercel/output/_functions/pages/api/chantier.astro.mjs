...
const GET = async ({ request }) => {
  ...
  const { data: chantiers, error: chantiersError } = await supabase.from("chantiers").select("id, nom, emoji, budget, phase, created_at, updated_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(10);
  if (chantiersError) {
  ...
  const { data: devisRows, error: devisError } = await supabase.from("devis_chantier").select("id, chantier_id, artisan_nom, type_travaux, montant_ttc, statut, score_analyse, analyse_id").in("chantier_id", chantierIds);
  if (devisError) {
  ...
  const { data: activiteRows } = await supabase.from("devis_chantier").select("id, artisan_nom, type_travaux, montant_ttc, statut, created_at, chantier_id").in("chantier_id", chantierIds).order("created_at", { ascending: false }).limit(5);
  const chantiersAvecDevis = chantiers.map((c) => ({
    ...c,
    devis: (devisRows ?? []).filter((d) => d.chantier_id === c.id).map((d) => ({
      id: d.id,
      nom: d.artisan_nom,
...
const POST = async ({ request }) => {
  ...
  const { data, error } = await supabase.from("chantiers").insert({
    ...
    phase: "preparation"
  }).select("id, nom, emoji, budget, phase, created_at, updated_at").single();
  if (error) {

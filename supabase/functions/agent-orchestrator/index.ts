...
serve(async (req) => {
  ...
  async function processChantier(chantierId: string): Promise<boolean> {
    ...
    if (runType === "evening") {
      ...
      // Le récit du journal = UNIQUEMENT le digest narratif de l'IA. Les
      // événements horodatés (décisions, alertes, changements de statut…) vivent
      // désormais dans la timeline du Journal (endpoint /journal/timeline) —

...
// Injecté depuis tools/index.ts : Set des tools valides — utilisé pour valider
// expected_action.tool dans notify_owner_for_decision (rejet si Gemini hallucine).
let _validToolNames: ReadonlySet<string> | null = null;
export function injectValidToolNames(names: ReadonlySet<string>) { _validToolNames = names; }

export const BATCH_SCHEMAS: Tool[] = [
  // notify_owner_for_decision est BATCH-SAFE : c'est une notif PRIVÉE au owner
  // (pas un envoi à un tiers), elle CRÉE une pending pour validation, ne fait
  // rien d'irréversible. Batch-safe = utilisable depuis morning/evening triggers
  // (msg artisan, upload doc, email entrant) — sinon le workflow "détection
  // décision à arbitrer" ne s'enclenche jamais.
  {
    type: "function",
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          expected_action: {
            type: "object",
            description: "Action à exécuter si l'owner répond OUI. Format { tool: 'shift_lot', args: {...} }. Le `tool` doit être un nom de tool existant.",
            properties: {
              tool: { type: "string", description: "Nom du tool à appeler (parmi les tools mutateurs disponibles)" },
...
// Note : notify_owner_for_decision a été déplacé en BATCH_SCHEMAS (haut du fichier)
// pour permettre son usage en mode morning/evening (workflow "détection décision artisan").
...
export const handlers: Record<string, Handler> = {
  ...
  notify_owner_for_decision: async ({ chantierId, args }) => {
    ...
    const expectedAction = args.expected_action as { tool?: string; args?: Record<string, unknown> } | undefined;
    ...
    // Valide expected_action.tool — refus si Gemini hallucine un nom de tool inconnu
    // (sinon on enverrait un WhatsApp avec une fausse promesse au user, qui répondrait
    // OUI, et resolve_pending_decision échouerait silencieusement à l'exécution).
    const toolName = String(expectedAction.tool ?? "");
    if (!toolName) {
      return JSON.stringify({ ok: false, error: "expected_action.tool requis" });
    }
    if (_validToolNames && !_validToolNames.has(toolName)) {
      return JSON.stringify({
        ok: false, error: "invalid_tool",
        message: `Tool '${toolName}' inconnu. Utilise un nom de tool existant pour expected_action.tool.`,
      });
    }
